...
def fts_spectrum(xs_raw,name,window_name,plot_path=None,stepsize=738./125000.,mode='s',
	...
	if mode == 'd':
		#combine the two sides of the interferogram (left then right side), with tail if desired
		if include_tail:
	...
	if not os.path.exists(os.path.join(output,'plots')):
		...
		cmd = 'mkdir '+os.path.join(output,'plots')
      		...
      		if ret != 0:
          		...
          		print ('Plots output directory ' + os.path.join(output,'plots') +
...
def plot_spectrum(data, style='-'):
	...
	pl.plot(fs,ys,style,label=name+' '+window+' %sth-order'%(poly))
