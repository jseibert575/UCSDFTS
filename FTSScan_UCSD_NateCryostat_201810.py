...
def FTS_Scan(dataname,mapfile,stageIP,stagePort,vel=1.0,accel=2.0,intTime=3,pauseTime=0.5,save=None):#,oldMap=False):
	...
	If a KeyboardInterrupt is found during the scan, the scan will stop and any data taken up to
	that point will be saved as normal.
	...
	# else:
	# 	mappoints = numpy.loadtxt(mapfile, skiprows = 4)
	mappoints = numpy.loadtxt(mapfile)
...
if __name__ == "__main__":
	...
	parser.add_argument('-d',dest='dataname',action='store',default=None,help='Name of dataset') #THIS IS NOT USED - SHOULD PROBABLY DELETE
	...
	parser.add_argument('-test',dest='test',action='store_true',help='Test run - ignores other input')
	#parser.add_argument('-oldMapFile', action = 'store_true', help='Called if using mapfile without a header')
	...
	# Parse the argument to find if the mapfile has a header
	# if args.oldMapFile:
	# 	oldMap = True
	# else:
	# 	oldMap = False
	...
	data = FTS_Scan(args.dataname,map_file,Mirror_Stage_IP,Mirror_Stage_Port,intTime=intTime,save=save_path)#, oldMap=oldMap)
